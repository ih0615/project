...
import shop.kkyochon.banban.csc.action.CustomerCenterAction;
import shop.kkyochon.banban.csc.action.EventAction;
import shop.kkyochon.banban.csc.action.NoticeAction;
import shop.kkyochon.banban.csc.action.RegisterQusFormAction;
import shop.kkyochon.banban.csc.action.RegisterQusProcAction;
...
public class CscController extends HttpServlet {
	...
	protected void doProcess(HttpServletRequest request, HttpServletResponse response)
		...
		}else if (command.equals("/registerQusForm")) {
			Action action = new RegisterQusFormAction();
			try {
				forward = action.execute(request, response);
			} catch (Exception e) {
				e.printStackTrace();
			}
		}else if (command.equals("/registerQusProc")) {
			Action action = new RegisterQusProcAction();
			try {
				forward = action.execute(request, response);
			} catch (Exception e) {
				e.printStackTrace();
